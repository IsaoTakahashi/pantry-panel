...
`.claude/rules/testing.md` の「判断基準の更新ログ」本体。テスト設計時に過去の判断根拠を確認する場合はここを参照する。新しいエントリの追記フォーマットは `.claude/rules/testing.md` の「判断基準の更新ログ」セクションのテンプレートに従う。
...
### 2026-09-10: e2e-preview の flaky率は「最新 attempt だけを見る集計」では過小評価される。rerun は「1回で必ず直る」わけではない

- **対象シナリオ:** realtime-sync.spec.ts（Issue #247、Issue #258/PR #259 の作業中に CI 実行履歴を根拠にした傾向分析を実施）
- **変更前:** 「Issue #247 は間欠的な軽微な不調で、rerun すれば通る」という経験則的な理解のみで、実数による裏付けはなかった
- **変更後:**
  1. `gh run list` はワークフロー run ID ごとに**最新 attempt の結論しか表示しない**。途中の attempt（rerun 前）が失敗していても、後の attempt が成功していればその run は単に "success" として見え、失敗が発生した事実そのものが集計から消える。flaky率を測るには `gh api repos/.../actions/runs/{id}/attempts/{n}` で attempt 単位の履歴を全数取得する必要がある
  2. 2026-06-22〜08-02 の約6週間、e2e-preview の失敗率が 94%（61失敗/65完了）という極端な期間があったが、これは Issue #247 とは別原因（`@playwright/test` の npm バージョンと CI Docker イメージ `v1.60.0-noble` の不一致により、全テストが起動2msで `browserType.launch: Executable doesn't exist` により即死）だった。renovate の依存更新 PR #220（2026-08-03マージ、Docker イメージを `v1.62.1-noble` に追従）で解消済み。この期間を Issue #247 の flaky率計算に含めると数値が大きく歪む
  3. 2026-08-03 以降（原因別の期間を除いたクリーンな母集団）の全78 run について attempt 単位で全数調査した結果、attempt 1 が失敗した run は25件（32.1%）。「19.7%」という中間報告（最終結論だけを見た集計）は rerun で救われたケースを見落として過小評価していた
  4. 実際に rerun された11件のうち、1回の rerun で直ったのは7件（64%）のみ。**2件は複数回の rerun が必要**（うち1件は6回 rerun して7回目でようやく成功）、**2件は最終的に rerun でも解決していない**（うち1件は4回 rerun して計5回連続失敗のまま放置）。「rerun すれば1回で必ず直る」という前回の中間報告は、少数サンプルからの誤った一般化だった
- **理由:** CI のフレーキーさを「大体rerunすれば直る」という感覚だけで運用し続けると、rerun しても直らないケースを見落とし続けるリスクがある。実際に25件中2件（8%）は複数回の rerun を要し、2件（8%）は放置されたまま解決していない——これは「間欠的な軽微な不調」より重い、未解決の根本原因の存在を示す直接証拠
- **一般化した基準:**
...
### 2026-09-11: 設計時の「どちらの分岐が red になるか」の予想は、モック構成次第で外れる。実測してから実装範囲を決める

- **対象シナリオ:** middleware.test.ts S-9（Issue #260、`isDefinitelyUnauthenticated` による redirect 時の cookie クリア）
- **変更前:** design.md の想定では、`isDefinitelyUnauthenticated` に至る2経路のうち「data===null && error===null」（本当に未ログイン）は `@supabase/ssr` が内部 `setAll()` で既に cookie をクリアしている可能性があり、「確定的無効エラー」経路のみ cookie が残ると予想していた
- **変更後:** 実装前に両経路の RED を実測したところ、**両方とも red**（cookie が残る）だった。理由は、`middleware.test.ts` が `createServerClient` 自体を丸ごとモックしているため、`@supabase/ssr` の実装が内部で `setAll()` を呼ぶかどうかはこのテストの守備範囲外（`getClaimsMock` は `setAll` を呼ばない）——つまり `middleware.ts` 自身は元々どちらの経路でも cookie を明示的にクリアしていなかった。修正は経路を問わず redirect 分岐全体に対して行う形になった
- **理由:** 「片方の分岐は外部ライブラリが既にカバーしているはず」という設計時の推測は、テストのモック境界（ここでは `createServerClient` を丸ごと差し替えている）次第で成立しなくなる。モックが外部ライブラリの内部動作まで再現していない場合、そのライブラリに依存した「既にクリアされているはず」という前提は自コードのテストからは検証できない
- **一般化した基準:**
  1. 設計時に「この分岐は既存の別コンポーネント（ライブラリ・別モジュール）が処理しているはず」という前提でスコープを絞る場合、実装前に实際のテスト環境（モック境界）でその前提が観測可能かを確認する。観測できない場合は「未検証の前提」として扱い、両方の分岐にテストケースを用意してから実装範囲を決める
  2. 設計ドキュメント（design.md）の予想が実測と異なった場合は、実装を予想に合わせて絞るのではなく実測に合わせて修正し、設計ドキュメント側もその場で更新する
