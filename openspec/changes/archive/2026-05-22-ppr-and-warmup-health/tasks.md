...
- [ ] 3.4 GitHub リポジトリの Settings → Secrets → `VERCEL_APP_URL` に本番 URL を登録する（手動作業）

## 4. 動作確認

- [x] 4.1 ローカルで `next build` を実行し、PPR ビルドがエラーなく完了することを確認する
- [ ] 4.2 `GET /api/health` をローカルで叩き、バックエンド起動中・停止中の両方でレスポンスを確認する
- [ ] 4.3 Vercel にデプロイして本番 URL で `/api/health` の疎通を確認する
- [ ] 4.4 GitHub Actions の keep-warm ジョブを `workflow_dispatch` で手動トリガーして成功することを確認する
