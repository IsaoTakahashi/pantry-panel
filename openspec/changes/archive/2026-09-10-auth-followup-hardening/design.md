...
さらに、独自キャッシュは「重複するだけ」に留まらず、**ローテーション安全性の観点ではライブラリ内蔵のキャッシュより悪化する**。当初案の `getClaims(undefined, { jwks: { keys } })` は `fetchJwk(kid, jwks)` の**最初の分岐**（呼び出し側が渡した `jwks` 内を探す）で完結し、ライブラリ自身の `GLOBAL_JWKS`（TTL チェック付き）にも、TTL 切れ時の即時再取得にも一切到達しない。つまり独自キャッシュを実装した場合、ロー­テーション後の新しい `kid` が独自キャッシュ内に無くても、ライブラリの「kid-miss で即時再取得」という安全機構がそもそも発火しないまま `jwk = undefined` となり得る——TTL を無視して確実に再取得するライブラリ内蔵の経路より、明確に安全性が劣る。この点も独自実装を見送る根拠に加える（最終ブランチ全体レビューで指摘、2026-09-10）。
