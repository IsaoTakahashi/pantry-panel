...
- [Trade-off] `useStockItemsRealtime.ts` の `.subscribe()` コールバックで `status === "SUBSCRIBED"` になるたびに `onChangeRef.current()`(全件再フェッチ)を呼ぶ(マウント直後の初回 SUBSCRIBED だけでなく、切断後の再接続で再度 `SUBSCRIBED` が発火した場合も同様に発火する)。`postgres_changes` は `SUBSCRIBED` 到達前の変更を再送しないため、動的 import 化(Decision 1)でマウント〜購読確立の間が広がった分、その間に発生した変更が再送されずに失われる可能性がある。この再フェッチはマウント〜購読確立までに来た変更を拾うため、この窓を「狭める」のではなく「閉じる」――ただし `onChange`(`useStockItems.ts` の `handleRealtimeChange`)自体は `accessToken`/`effectiveGroupId` が未解決のまま呼ばれると黙って失敗する(エラーは `.catch(() => {})` で握りつぶす)ため、無条件に成功するわけではない。その場合の取りこぼしは、値解決後に走る本来のフェッチ effect 側でカバーされる。コストは `useStockItemsRealtime` マウント・再接続ごとに `GET /api/stock-items` が1回増えること(`#217` の over-fetch 削減、および `d91cabc` が無関係な storm 修正の一環として除去した挙動への部分的な巻き戻し)。一度 `ce2bab2` で適用 → `cd0e45b` で revert されたが、その revert 判定は E2E `.serial` retry のテストハーネス汚染(2026-09-04 の `testing.md` 更新ログ参照)で汚染された偽陰性シグナルに基づいており無効。再フェッチ到達後に残る「`SUBSCRIBED` 応答〜サーバー側 WAL バインディングが実際に有効になるまで」の窓はクライアントから観測・修正不能なため許容する(Issue #247)
- [Trade-off] `getSupabaseClient()`(async)に加えて `peekSupabaseClient()`(同期、未解決なら `undefined`)を追加し、`useStockItemsRealtime.ts` は解決済みならその場で同期的に `.subscribe()` する(未解決なら従来どおり `getSupabaseClient().then(...)` の非同期パス)。狙いは main ブランチの完全同期な subscribe とタイミングを揃え、マウント〜購読確立の窓自体を(閉じるのではなく)可能な限り短くすること。**設計時の前提訂正**: 当初「`AuthGuard` 配下でレンダリングされるため `session` 確定時点で `getSupabaseClient()` は必ず解決済み」という前提で書かれたが、レビューで誤りと判明 —— `useStockItemsRealtime` は `StockItemsClient` のトップレベルで無条件に呼ばれ、`AuthGuard` の children としてではないため、この保証は成立しない。ただし 2026-09-04 の CI 実測（同一 window で2回計測、計6購読中5回が sync パス、失敗した2ケースはいずれも async パスと相関)では、動的 import が module 評価時に発火することもあり実際には sync パスが多くのケースで発火することを確認した。保証ではなく実測に基づく改善であり、async パスに落ちた場合の残存リスクは上記の SUBSCRIBED-refetch と Issue #247 の対象のまま
