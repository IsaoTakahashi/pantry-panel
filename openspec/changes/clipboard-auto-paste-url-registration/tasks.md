...
- [x] 3.1 `navigator.clipboard.readText` のモック設定を `beforeEach` / `vi.fn()` で用意する
- [x] 3.2 **シナリオ: 有効URL → 自動submit開始** のテストを追加する
  - クリップボードに `https://example.com/product` をセット
  - モーダルを open
  - `extractFromUrlStream` が呼ばれること（自動submit）を検証
- [x] 3.3 **シナリオ: 非URLテキスト → 通知表示** のテストを追加する
  - クリップボードに `"何かのテキスト"` をセット
  - モーダルを open
  - "URLの読み取りに失敗しました" が表示されること
  - 読み取れた文字列が表示されること
- [x] 3.4 **シナリオ: 60文字超の非URLテキスト → 60文字＋省略記号** のテストを追加する
- [x] 3.5 **シナリオ: clipboard 読み取り失敗 → エラー通知のみ** のテストを追加する
  - `readText` が reject する状況をモック
  - "URLの読み取りに失敗しました" が表示され、clipboard テキストが表示されないことを検証
- [x] 3.6 **シナリオ: クリップボードが空 → 通知なし** のテストを追加する
- [x] 3.7 **シナリオ: navigator.clipboard が undefined → 通知なし・通常open** のテストを追加する
- [x] 3.8 **シナリオ: 通知表示後にユーザーが入力すると通知消去** のテストを追加する
- [x] 3.9 既存テストが全て pass することを確認する（`npm test` or `npx vitest run`）

## 4. E2Eテスト — url-registration.spec.ts

- [x] 4.1 **シナリオ: クリップボードに有効URLを設定してリンクボタンをクリック → 自動submit** のE2E Mockテストを追加する
...
- [x] 4.2 既存E2Eテストが全て pass することを確認する（`npx playwright test`、dev server 起動済み）
- [x] 4.3 E2Eテスト修正が必要な場合は `not.toBeAttached()` パターンで対応する（モーダルのexit animation対策）
