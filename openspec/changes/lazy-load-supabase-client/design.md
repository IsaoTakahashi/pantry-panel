...
- [Risk] (Decision 1 のトレードオフ節で詳述)cold 訪問時に `accessToken` 判明が静的 import 時より遅れる可能性がある → `#236` の実測ハーネス(storageState + Playwright、warm シナリオ)を**このPRの Vercel Preview URL に対して**再実行し(`E2E Preview` CI ジョブが Preview URL を用意する)、その実行内で `GET /api/stock-items` が `GET /api/groups/me` の応答を待たずに開始される構造(＝ `#236` が獲得した性質)が維持されているかを確認する。ローカル(`localhost`)の実測値は本番 `#236` の記録値(`groups req@~110ms` 等、Issue #236 コメント参照)と環境が異なり比較不能なため使わない。post-merge で本番に対して再計測し、Issue #238 に記録する(タスク7参照)。有意な後退があればタスク未完了として扱う(フォールバック: `ReactDOM.preinit` 相当の先読み)
