...
import StockItemsClient from "@/app/stock-items/StockItemsClient";
...
const { mockSignOut, mockAuthState } = vi.hoisted(() => ({
  mockSignOut: vi.fn(),
  // mutable so individual tests can flip `loading` (authLoading) to exercise
  // the D5 skeleton-gate fix (`authLoading && initialItems === null`) without
  // affecting the other 27 tests, which all rely on loading: false.
...
describe("StockItemsClient", () => {
  ...
  it("authLoading が true でも initialItems があればスケルトンをスキップし一覧を表示する（D5）", async () => {
    // 他の27件は AuthContext モックの loading: false に依存しているため、この
    // gate（authLoading && initialItems === null）の authLoading=true 分岐は
    // どのテストでも実際には通っていない。mockAuthState.loading を明示的に
    // true にして初めてこの分岐を検証できる。
    mockAuthState.loading = true;
    const { fetchStockItems } = await import("@/lib/api");
    const initialItems = [
      {
        id: "1",
        name: "SSR商品（authLoading中）",
        category: "調味料",
        imageUrl: null,
        sourceUrl: null,
        wantToBuy: false,
    ...
    expect(screen.getByText("SSR商品（authLoading中）")).toBeInTheDocument();
    expect(fetchStockItems).not.toHaveBeenCalled();
  });
});
