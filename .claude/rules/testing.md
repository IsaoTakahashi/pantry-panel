...
- **一般化した基準:**
  ...
  4. **追記（同日）**: この時点では根本原因不明として retry で許容する方針だったが、直後に根本原因が判明した（Issue #247 参照）。E2E テストユーザーの `group_members` が263件まで無制限に蓄積しており（`global-setup.ts` が CI 実行毎に新規 group を作成する一方 `DELETE /api/groups/:id` 未実装で片付けられていなかった）、Supabase Realtime の `apply_rls` 評価がこの行数で破綻し新規 subscribe でのイベント配信が失われていた。最新1件を残して DB を直接クリーンアップした直後、追加のコード変更なしで CI が安定 green に戻った。retry 許容（上記1-3）は妥当な設計判断として残すが、**E2E 用に動的作成するリソースは "作りっぱなし" にせず片付ける経路を用意する**（`DELETE /api/groups/:id` の実装、`global-teardown.ts` での group 本体の削除）ことを合わせて追跡する。「根本原因の追跡はコストに見合わない」という判断は、蓄積型の原因（行数・接続数など）を見落としやすいので早期に疑う
