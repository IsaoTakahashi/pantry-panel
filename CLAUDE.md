...
**ブランチ運用（必須）:**
- **ドキュメント変更を含む、プロジェクトへのすべての変更はブランチを切り、PR 経由で main にマージする。main への直接コミット・push は禁止。**
- ブランチ名: `{issue番号}-{概要}`（例: `71-filter-animation`）
- 作業前に必ず GitHub Issue を作成し、ブランチ名に番号を含める
- PR 本文に `Closes #N` を付けてマージ時に Issue を自動クローズする
